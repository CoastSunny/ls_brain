function [res,Cs,fIdxs]=cvtrainFn2(objFn,X,Y,Cs,fIdxs,varargin)
...
opts = struct('binsp',1,'aucNoise',0,'recSoln',0,'dim',[],...
              ...
              'subIdx',[],'aucWght',.1,'fpr',[],'tpr',[],'w_template',[]);
   ...
   for ci=1:size(Cs,2);%siCs; % proc in sorted order
      ...
      if( ~isempty(seed) ) 
         [seed,f,J]=feval(objFn,X,Ytrn,Cs(:,ci),'verb',opts.verb-1,...
                          opts.seedNm,seed,'dim',opts.dim,varargin{:},'w_template',opts.w_template);
      else
         [seed,f,J]=feval(objFn,X,Ytrn,Cs(:,ci),'verb',opts.verb-1,...
                          'dim',opts.dim,varargin{:},'w_template',opts.w_template);
      end
