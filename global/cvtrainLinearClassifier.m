function [classifier,res]=cvtrainLinearClassifier(X,Y,Cs,fIdxs,varargin)
...
opts=struct('objFn','klr_cg','dim',[],'spType','1v1','spKey',[],'spMx',[],'zeroLab',0,...
            'balYs',0,'verb',0,'Cscale',[],'fIdxs',[],'calibrate','cr');
...
if ( isempty(Cs) ) Cs=[5.^(5:-1:-5) 0]; end;
...
res=cvtrainFn(opts.objFn,K,Y,Cscale*Cs,fIdxs,'dim',numel(dim)+(1:numel(dim)),varargin{:},'calibrate',opts.calibrate); 
res.Y=Y;
