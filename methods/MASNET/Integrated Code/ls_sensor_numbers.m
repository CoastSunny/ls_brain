...
for pfa_idx=1:numel(pfa)
    ...
    for i=1:1:S_
        p_sep_d=[];p_div_d=[];p_3best_d=[];
        for j=1:96
            
            idx = randperm(S_,i);
            p_sep = calculating_Prob_detection_v2(AC_sample,Td,Tc,ALL_snr(:,:,:,idx),pfa(pfa_idx));
            p_div = calculating_Prob_detection_v2(AC_sample,Td,Tc,sum(ALL_snr(:,:,:,idx),4),pfa(pfa_idx));
...
%         if i>=4;p3best_d(i,pfa_idx)=mean(mean(p_3best_d));end;
 ...
 filename3 = ['~/Documents/projects/ls_brain/results/Probs_CORRSHD_TS_' num2str(Type_Scenario)...
