...
for pfa_idx=1:numel(pfa)
    ...
    for i=1:1:S_
        ...
        for j=1:96 %number of perms
            ...
            idx = randperm(S_,i);
            p_all = calculating_Prob_detection_No_CP(AC_sample,ALL_snr(:,:,:,idx,1:tm),pp);
            p_mean = calculating_Prob_detection_No_CP(AC_sample,mean(ALL_snr(:,:,:,idx,1:tm),4),pp);
            p_sum= calculating_Prob_detection_No_CP(AC_sample,sum(ALL_snr(:,:,:,idx,1:tm),4),pp);
            if size(p_all,5)>1
...
%         if i>=4;p3best_d(i,pfa_idx)=mean(mean(p_3best_d));end;
  ...
  filename3 = ['~/Documents/projects/ls_brain/results/masnet/probs/Probs_CORRSHD_NOCP_Time_' num2str(tm) '_TS_' num2str(Type_Scenario)...
