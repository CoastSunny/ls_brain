...
#pragma endregion // check input validity
	...
	if (nlhs<=1) {
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = IsMemberSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz); break;
			case mxUINT64_CLASS		:	plhs[0] = IsMemberSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz); break;
		...
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = IsMemberSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz,plhs[1]); break;
			case mxUINT64_CLASS		:	plhs[0] = IsMemberSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz,plhs[1]); break;
			default					:	mexErrMsgTxt("Unsapported data type");
		}
	}
}
...
template <typename T> mxArray* IsMemberSorted( mxClassID classid,const T* ap,  const mwSize asz,const T* bp,  const mwSize bsz, mxArray*& pai)
{
	mxArray* Result = mxCreateLogicalMatrix(1,asz);  // initialized to 0
		...
		unsigned INT64T* ppai= reinterpret_cast<unsigned INT64T*>(mxGetPr(pai));
	#else  // 32 bit compilation
