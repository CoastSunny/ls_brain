...
#pragma endregion // check input validity
	...
	if (nlhs<=1) {
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = SetDiffSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz); break;
			case mxUINT64_CLASS		:	plhs[0] = SetDiffSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz); break;
		...
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = SetDiffSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz,plhs[1]); break;
			case mxUINT64_CLASS		:	plhs[0] = SetDiffSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz,plhs[1]); break;
			default					:	mexErrMsgTxt("Unsapported data type");
		}
	}
}
...
bool ProcessEmptyInput(const mxArray* ap, const mxArray* bp,int nlhs, mxArray *plhs[])
{
	...
	if (mxIsEmpty(ap)) { // a is empty
		 ...
		 plhs[0] = mxDuplicateArray(ap);
			...
			unsigned INT64T* pplhs_1 = (unsigned INT64T*)mxGetPr(plhs[1]);
			for (unsigned INT64T i = 0; i < mxGetNumberOfElements(ap); ++i) {
				pplhs_1[i] = (i+1);
			}
		#else  // 32 bit compilation
...
template <typename T> mxArray* SetDiffSorted( mxClassID classid,const T* ap,  const mwSize asz,const T* bp,  const mwSize bsz, mxArray*& pai)
{
	...
	if (CurrentBufferSize!=0) {memcpy((void*)mxGetPr(Result),ResultsBuffer,sizeof(T)*CurrentBufferSize);}
		...
		memcpy((void*)mxGetPr(pai),ais,sizeof(unsigned INT64T)*CurrentBufferSize);
	#else  // 32 bit compilation
