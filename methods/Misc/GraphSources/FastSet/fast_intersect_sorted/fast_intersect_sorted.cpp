...
#pragma endregion // check input validity
	...
	if (nlhs<=1) {
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = IntersectSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz); break;
			case mxUINT64_CLASS		:	plhs[0] = IntersectSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz); break;
		...
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = IntersectSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz,plhs[1],plhs[2]); break;
			case mxUINT64_CLASS		:	plhs[0] = IntersectSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz,plhs[1],plhs[2]); break;
			default					:	mexErrMsgTxt("Unsapported data type");
		}
	}
}
...
template <typename T> mxArray* IntersectSorted( mxClassID classid,const T* ap,  const mwSize asz,const T* bp,  const mwSize bsz, mxArray*& pai,mxArray*& pbi)
{
	...
	memcpy((void*)mxGetPr(Result),ResultsBuffer,sizeof(T)*CurrentBufferSize);
		...
		if (CurrentBufferSize!=0) { memcpy((void*)mxGetPr(pai),ais,sizeof(unsigned INT64T)*CurrentBufferSize); }
		pbi = mxCreateNumericMatrix(1,CurrentBufferSize,mxUINT64_CLASS,mxREAL);
		if (CurrentBufferSize!=0) {memcpy((void*)mxGetPr(pbi),bis,sizeof(unsigned INT64T)*CurrentBufferSize); }
	#else  // 32 bit compilation
