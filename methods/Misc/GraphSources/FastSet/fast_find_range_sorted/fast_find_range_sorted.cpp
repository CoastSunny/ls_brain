...
#pragma endregion // check input validity
	...
	if (nlhs<2) {
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = FindRangeSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz_m,bsz_n); break;
			case mxUINT64_CLASS		:	plhs[0] = FindRangeSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz_m,bsz_n); break;
		...
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = FindRangeSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz_m,bsz_n,plhs[1]); break;
			case mxUINT64_CLASS		:	plhs[0] = FindRangeSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz_m,bsz_n,plhs[1]); break;
			default					:	mexErrMsgTxt("Unsapported data type");
		}
	}
	if (nlhs>1) {
		...
		#ifdef _M_X64
			const INT64T *pIndex = (const  INT64T*)mxGetPr(plhs[0]);
		#else
		const int *pIndex = (const int*)mxGetPr(plhs[0]);
		#endif
		for (unsigned INT64T i = 0 ; i < bsz_m; ++i) {
...
template <typename T> mxArray* FindRangeSorted( mxClassID classid,const T* ap,  const mwSize asz,const T* bp,  const mwSize bsz_m, const mwSize bsz_n)
{
	if (asz==0 || bsz_m==0 || bsz_n==0) { return mxCreateNumericMatrix(0,0,classid,mxREAL); } // should never be triggered . see ProcessEmptyInput	
		...
		INT64T *pResult =(INT64T*)mxGetPr(Result);
		INT64T p_start, p_end;
	#else 
