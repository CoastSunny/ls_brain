...
#pragma endregion // check input validity
	...
	if (nlhs<=1) {
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = XORSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz); break;
			case mxUINT64_CLASS		:	plhs[0] = XORSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz); break;
		...
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = XORSorted(classid,(const INT64T*)ap,asz,(const INT64T*)bp,bsz,plhs[1],plhs[2]); break;
			case mxUINT64_CLASS		:	plhs[0] = XORSorted(classid,(const unsigned INT64T*)ap,asz,(const unsigned INT64T*)bp,bsz,plhs[1],plhs[2]); break;
			default					:	mexErrMsgTxt("Unsapported data type");
		}
	}
}
...
bool ProcessEmptyInput(const mxArray* ap, const mxArray* bp,int nlhs, mxArray *plhs[])
{
	...
	if (nlhs>1) {
			...
			unsigned INT64T* aip = (unsigned INT64T*)mxGetPr(plhs[1]);
			for (unsigned INT64T i = 0; i < mxGetNumberOfElements(plhs[1]); ++i) {
				aip[i] = (i+1);
			}
		...
		if (nlhs>2) {
			...
			unsigned INT64T* bip = (unsigned INT64T*)mxGetPr(plhs[2]);
			for (unsigned INT64T i = 0; i < mxGetNumberOfElements(plhs[2]); ++i) {
				bip[i] = (i+1);
			}
		#else  // 32 bit compilation
...
template <typename T> mxArray* XORSorted( mxClassID classid,const T* ap,  const mwSize asz,const T* bp,  const mwSize bsz, mxArray*& pai,mxArray*& pbi)
{
	...
	memcpy((void*)mxGetPr(Result),ResultsBuffer,sizeof(T)*CurrentBufferSize);
		...
		if (CurrentBufferSize!=0) { memcpy((void*)mxGetPr(pai),ais,sizeof(unsigned INT64T)*ais_i); }
		pbi = mxCreateNumericMatrix(1,bis_i,mxUINT64_CLASS,mxREAL);
		if (CurrentBufferSize!=0) {memcpy((void*)mxGetPr(pbi),bis,sizeof(unsigned INT64T)*bis_i); }
	#else  // 32 bit compilation
