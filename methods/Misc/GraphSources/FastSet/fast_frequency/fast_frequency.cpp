...
#pragma endregion // check input validity
	...
	if (nlhs<=1) {
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = Frequency(classid,prhs[0],(const INT64T*)ap,asz); break;
			case mxUINT64_CLASS		:	plhs[0] = Frequency(classid,prhs[0],(const unsigned INT64T*)ap,asz); break;
		...
		switch (classid) {
			...
			case mxINT64_CLASS		:	plhs[0] = Frequency(classid,prhs[0],(const INT64T*)ap,asz,plhs[1]); break;
			case mxUINT64_CLASS		:	plhs[0] = Frequency(classid,prhs[0],(const unsigned INT64T*)ap,asz,plhs[1]); break;
			default					:	mexErrMsgTxt("Unsapported data type");
		}
	}
}
...
template <typename T> mxArray* Frequency( mxClassID classid,const  mxArray* A,const T* ap,  const mwSize asz, mxArray*& pIDs)
{
	...
	pIDs = plhs[0];
		...
		unsigned INT64T* pResult = (unsigned INT64T*)mxGetPr(Result);
		const INT64T* pIndexes = (const INT64T*)mxGetPr(plhs[2]);
	#else  // 32 bit compilation
