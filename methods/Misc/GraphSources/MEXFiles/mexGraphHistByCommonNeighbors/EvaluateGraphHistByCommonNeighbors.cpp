#include "stdafx.h"
#include "EvaluateGraphHistByCommonNeighbors.h"
#include <iostream>
#include <fstream>
#include <math.h>
#include <map>
...
void	FreeResourses(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	for (unsigned int i = 0; i < MaxActualDistance; ++i) {
		mxSetField(Result,i,chDistance,mxCreateScalarDouble(i+1) );
		mxSetField(Result,i,chNumberOfBoxes,mxCreateScalarDouble(0.0) );
	}
...
void DisplayProgressUpdate(unsigned int Progress, unsigned int Total)
{
	if (Progress==0) { // initialize
		...
		if (ShowProgress) {
			...
			sprintf_s(DummyStr,"Progress: Node %d of %d, (%d%%), Elapsed: %fsec. Left: %f.", Progress,Total,int((100.0*Progress)/Total+0.5),ElapsedTime,ToGoTime);
