...
#include "EvaluateGraphLinkDirectionality.h"
...
struct LinkData {
	...
	LinkData(unsigned int ID) :DestinationNodeID(ID),InwardPathCount(0),AtwardPathCount(0),OutwardPathCount(0) {}
	unsigned int DestinationNodeID;
	unsigned int InwardPathCount;
	unsigned int AtwardPathCount;
	unsigned int OutwardPathCount;

	bool operator==(const LinkData& rhs) const { return DestinationNodeID==rhs.DestinationNodeID;}
	bool operator<(const LinkData& rhs) const { return DestinationNodeID<rhs.DestinationNodeID;}

};
std::vector< std::set<LinkData> > LinkPathData;
...
{	
	...
	LinkPathData.resize(Graph.GetMaxNodeID()+1);
}
//------------------------------------------------------------------------------------------------------------------
	...
	if (CurrentDistance==0) {  
		...
		for(std::set<unsigned int>::const_iterator it = nodeNeighbours[CurrentDistance-1].begin(); it!=nodeNeighbours[CurrentDistance-1].end(); ++it) {
			...
			for (std::vector<unsigned int>::const_iterator CurrNeighbour = Neighbours.begin(); CurrNeighbour != Neighbours.end(); ++CurrNeighbour) {
				std::set<LinkData>::iterator CurrentLinkPathData = LinkPathData[*it].find(*CurrNeighbour);				
				if (CurrentLinkPathData==LinkPathData[*it].end()) {
					std::pair <std::set<LinkData>::iterator, bool> InsertResult = LinkPathData[*it].insert(LinkData(*CurrNeighbour));
					CurrentLinkPathData  = InsertResult.first;
				}
				if (nodeDistances[*CurrNeighbour]==-1) {
					...
					const_cast<LinkData&>(*CurrentLinkPathData).OutwardPathCount = (*CurrentLinkPathData).OutwardPathCount+1;  // http://stackoverflow.com/questions/3354346/stl-iterators-and-const
				}
					...
					if (nodeDistances[*CurrNeighbour]==CurrentDistance-1) { // notice, in undirected network, nodeDistances[*CurrNeighbour] may only be (CurrentDistance-1)
						...
						const_cast<LinkData&>(*CurrentLinkPathData).AtwardPathCount = (*CurrentLinkPathData).AtwardPathCount+1;
					}
						...
						const_cast<LinkData&>(*CurrentLinkPathData).InwardPathCount = (*CurrentLinkPathData).InwardPathCount+1;
					}
				}
			}
		}
...
void	FreeResourses(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
	...
	const char* chLinksData = "LinksData";
	
	const char* FieldNames[] = { chKCCData,chKCCDataAverage,chKCCSummary,chLinksData };
	...
	// copy links data into output.
	mxArray* mxLinksData = mxCreateDoubleMatrix(NumberOfLinksInGraph,5,mxREAL);
	double* pLinksData  = mxGetPr(mxLinksData);
	unsigned int CurrentIndex = 0;
	for (std::vector< std::set<LinkData> >::const_iterator It1= LinkPathData.begin();It1!=LinkPathData.end();++It1) {
		for (std::set<LinkData> ::const_iterator It2=It1->begin();It2!=It1->end(); ++It2) {
			pLinksData[CurrentIndex]							= (unsigned int)(It1-LinkPathData.begin());
			pLinksData[CurrentIndex+NumberOfLinksInGraph]		= It2->DestinationNodeID;
			pLinksData[CurrentIndex+2*NumberOfLinksInGraph]		= It2->InwardPathCount;
			pLinksData[CurrentIndex+3*NumberOfLinksInGraph]		= It2->AtwardPathCount;
			pLinksData[CurrentIndex+4*NumberOfLinksInGraph]		= It2->OutwardPathCount;

			++CurrentIndex;
		}
	}

	mxSetField(Result,0,chLinksData,mxLinksData);
