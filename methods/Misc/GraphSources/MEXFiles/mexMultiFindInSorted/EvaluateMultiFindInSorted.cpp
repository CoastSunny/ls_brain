...
template <typename T> void DoFind(unsigned int* CurRes, const T* const pSearch, const unsigned int SearchSize, const T* const pData, const unsigned int DataSize) 
{
	...
	const T*	CurSearch = pSearch;
	const T*	CurData	= pData;

	while (CurSearch < pSearch + SearchSize) {
		for ( ;CurData<pData+DataSize && *CurData < *CurSearch; ++CurData);
		if (CurData < pData+DataSize && *CurData == *CurSearch) {
...
{
	memset(CurRes,0,SearchSize*sizeof(unsigned int));
	unsigned int CurrSearchIndex = 0;
	unsigned int CurrDataIndex = 0;

	while (CurrSearchIndex < SearchSize) {
		const mxArray* const CurSearch = mxGetCell(pSearch,CurrSearchIndex);
		for (; CurrDataIndex < DataSize && *CurRes==0;   ++CurrDataIndex) {
...
{	
	...
	if ( mxIsCell(pResult) ) {
		...
		for (unsigned int i = 0; i < mxGetN(pData); ++i) {
			...
			mexCallMATLAB(1, plhs, 2,prhs, "mexMultiFindInSorted");
