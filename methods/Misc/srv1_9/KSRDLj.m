function [AtA,Y,A,pinvY,C,rr,numIter,tElapsed,finalResidual]=KSRDLj(Xi,Xs,C,XtX,k,option)
    ...
    switch option.dicPrior
        case 'uniform'
            if c==k
                ...
                if Xtf
%                     A=X*pinvY;
                    A=inv(pen*eye(size(C'*C))+C'*C)*(pen*Xi+C'*Xs)*pinvY;
                end
                AtA=A'*A;
                AtX=A'*(pen*Xi+C'*Xs);      
%                AtX=pinvY'*XtX;
...
%                 AtX=pinvY'*XtX;
            ...
            B=pen*A'*A+A'*(C'*C)*A;
            Z=A'*(pen*Xi+C'*Xs);
            XtX=(pen*Xi+C'*Xs)'*(pen*Xi+C'*Xs);
            [AtA,~,XtX,~,AtX]=normalizeKernelMatrix(AtA,XtX,AtX);
            [B,~,XtX,~,Z]=normalizeKernelMatrix(B,XtX,Z);
...
%     end
%     XTX=(A'*(Xi+C'*Xs))*(Xi+C'*Xs)'*A;

    Y=KSRSC(B,Z,diag(XtX),option);
%     C=Xs*Y'*A'*inv(A*Y*Y'*A'+0.0*eye(size(A*Y*Y'*A'))*norm(A*Y*Y'*A','fro'));
    C=Xs*pinv(A*Y);
    %  Cs=Xs*(A*Y)'*inv(A*Y*(A*Y)'+100*eye(size(A*Y*(A*Y)')));
%     C=Xs*Y'*pinv(A*Y*Y');
%      C=normc(C);
     %C=C/diag(sum(C));
...
%     end
%     
    curRes1=norm(Xi-A*Y,'fro')+norm(Xs-C*A*Y,'fro');
    curRes2=norm(Xi-A*Y,'fro');
    curRes3=norm(Xs-C*A*Y,'fro');
    ...
    if mod(i,50)==0 || i==option.iter
        ...
        curRes=norm(Xi-A*Y,'fro')+norm(Xs-C*A*Y,'fro');        
