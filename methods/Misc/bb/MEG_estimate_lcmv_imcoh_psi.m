function MEG_estimate_lcmv_imcoh_psi(ndatasets, dataset_string)
...
Lred = [];
for ivox = 1:size(L, 2)
  [U_ S_ V_] = svd(squeeze(L(:, ivox, :)), 'econ');
  Lred(:, ivox, :) = U_(:, 1:2);
end
L = Lred;
...
for idata = 1:ndatasets
    ...
    load(['data/' dataset_string '/MEG/dataset_' num2str(idata) '/data']);
    
    MEG_data = MEG_data(elind, :);
    MEG_baseline_data = MEG_baseline_data(elind, :);
    ...
    % baseline
    [csb, cohb, ~]=data2cs_event(MEG_baseline_data', segleng, segshift, epleng, maxfreqbin, para);
...
%     % save predictions
%     save(['data/' dataset_string '/MEG/dataset_' num2str(idata) '/est_' mfname], 'est')
