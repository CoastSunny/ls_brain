function [A,H,C,P,M,ev,AddiOutput]=parafac2(X,F,Constraints,Options,A,H,C,P);
...
if nargin<5
   if initi==0
      ...
      Opt(4) = 0;
      Opt(5) = 1;
      [A,H,C,P,M,bestfit]=parafac2(X,F,Constraints,Opt);
      AllFit = bestfit;
      for i = 2:NumRep
         Opt(3) = 2;   % Init with random
         [a,h,c,p,m,fit]=parafac2(X,F,Constraints,Opt);
         AllFit = [AllFit fit];
         if fit>bestfit
            A=a;H=h;C=c;P=p;
            bestfit = fit;
...
%           C(:,3)=3*exp(-(t-16).^2/2^2);
%           C(:,4)=3*exp(-(t-16).^2/2^2);
