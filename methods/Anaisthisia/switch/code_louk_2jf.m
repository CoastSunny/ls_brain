...
for si=1:10;%[1:3  5:10];%1:numel(subjects)
    ...
    if ( isfield(z,'outfIdxs') ) % compute outer fold performance info
        z.prep(end).info.res.opt.res = cvPerf(z.Y,z.prep(end).info.res.tstf(:,:,Ci),[1 2 3],z.outfIdxs,'bal');
    ...
    % y=Y{si};
    % fpr=sum(tmp(outfIdxs{si}==1,1)>=0 & y(outfIdxs{si}==1,1)==1);
    % tpr=sum(tmp(outfIdxs{si}==1,1)<0 & y(outfIdxs{si}==1,1)==-1);
    % Res_Im(si,:)=[fpr tpr];
    % fpr=sum(tmp(outfIdxs{si}==1,2)>=0 & y(outfIdxs{si}==1,2)==1);
    % tpr=sum(tmp(outfIdxs{si}==1,2)<0 & y(outfIdxs{si}==1,2)==-1);
    % Res_Am(si,:)=[fpr tpr];
