...
dmsav=dmsavAd;
dmsav2=dmsavAd2;
dmsav_switch=dmsavAd_switch;
dmsav_switch2=dmsavAd_switch2;
for si=1:numel(dmsavA)%[1 2 3 4 5]% 6:10]%[9 5 1 2 3 4 6 7 8 10]
  ...
  %  idx_fpr=find(out{si}.out.bias<=bias_target);
    idx_fpr=find(dmsav{si}.out.bias<=-bias_savA(si));
%
    if (empirical==1)
        idx_fpr=find(dmsav2{si}.out.fpr.sav(m2,:)<=fpr_target);
    end
    % out{si}.out.bias(idx_fpr(end))-out{si}.out.bias(idx_fpr(end)-cheating_bias)
%     out{si}.out.bias(idx_fpr(end)-cheating_bias)
    amfpr2(si)=dmsav2{si}.out.fpr.sav(m2,idx_fpr(end)-cheating_bias);
    amtpr2(si)=dmsav2{si}.out.tpr.sav(m2,idx_fpr(end)-cheating_bias);
  %  amfpr3(count)=out{si}.out.fpr.sav3(m2,idx_fpr(end)-cheating_bias);
    amfpr(si)=dmsav{si}.out.fpr.sav(m2,idx_fpr(end)-cheating_bias);
    amtpr(si)=dmsav{si}.out.tpr.sav(m2,idx_fpr(end)-cheating_bias);
...
% %     stdDVCp(count)=std(dv_posC{count});
     ...
     amtpr_switch{si}=cell2mat(dmsav_switch{si}.out.tpr.sav(m2,idx_fpr(end)-cheating_bias,:));
     ...
     amtpr_switch2{si}=cell2mat(dmsav_switch2{si}.out.tpr.sav(m2,idx_fpr(end)-cheating_bias,:));
