...
trans=1;
axis_shift=0.1;
Yticklab={'-0.15' '-0.10' '-0.05' '0' '0.05' ' ' '-0.05' '0' '0.05' '0.10' '0.15'};
...
figure,hold on,shadedErrorBar(t,axis_shift+mean(OTA(tt,:),2),[ std(OTA(tt,:),0,2) zeros(numel(tt),1)],Oshaded_properties,trans),...
    shadedErrorBar(t,-axis_shift+mean(HTA(tt,:),2),[zeros(numel(tt),1) std(HTA(tt,:),0,2)],Dshaded_properties,trans)
axis(ax);
...
figure,hold on,shadedErrorBar(t,axis_shift+mean(OTI(tt,:),2),[ std(OTI(tt,:),0,2) zeros(numel(tt),1)],Oshaded_properties,trans),...
    shadedErrorBar(t,-axis_shift+mean(HTI(tt,:),2),[zeros(numel(tt),1) std(HTI(tt,:),0,2)],Dshaded_properties,trans)
axis(ax);
xlabel('time in seconds')
ylabel('normalised concentration change')
