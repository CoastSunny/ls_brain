...
% 
% for asd=1:size(Xi,2)
%     tmp2(asd)=norm(C*Xi(:,asd)+N(:,asd));
% end
% Xs=normc(Xs);
% 
% % Xs=eye(size(Xs,1),size(Xi,1))*Xi+N;Xs=normc(Xs);
% C=C/mean(tmp2);
% Xs=Xs+N;Xs=normc(Xs);
%Xs=normc(Xs);
...
%CC=Do*pinv(Dx);
CC=Xs(:,trn)*pinv(Dx*Yx);%CC=CC/norm(CC,'fro');
CC=C;
...
Dxc=CC*Dx;
nc=diag(Dxc'*Dxc);nd=diag(Dx'*Dx);
Dx=ones(Ni,1)*(nd./nc)'.*normc(Dx);
Dxc=normc(Dxc);
DtDxc=Dxc'*Dxc;
DtXx=Dxc'*Xs(:,tst);
...
%C=CC;
%pen=0.4;
%C=rand(size(Xs,1),size(Xi,1));
Xh=[sqrt(pen)*Xs;Xi];%Xh=normc(Xh);
H=computeKernelMatrix(Xh(:,trn),Xh(:,trn),optionKSRDL);
[DtD,Y,D,Dh,pinvY,Cs,Ci,rr]=KSRDLj4(Xi(:,trn),Xs(:,trn),C,Ci,pen,H,k,optionKSRDL); % Y is the representation of the training set in the feature space, the dictionry A is only meaningful for linear kernel. Usually we use AtA.
%D=Dh(38:end,:);
nc=diag(Dh(1:Ns,:)'/sqrt(pen)*Dh(1:Ns,:)/sqrt(pen));nd=diag(Dh(Ns+1:end,:)'*Dh(Ns+1:end,:));
...
Xh=[sqrt(pen)*Xs;Xi];%Xh=normc(Xh);
H=computeKernelMatrix(Xh(:,trn),Xh(:,trn),optionKSRDL);
[DtDy,Yy,Dy,pinvY,numIter,tElapsed,rry]=KSRDL(Xh(:,trn),H,k,optionKSRDL); % Y is the representation of the training set in the feature space, the dictionry A is only meaningful for linear kernel. Usually we use AtA.
nc=diag(Dy(1:Ns,:)'/sqrt(pen)*Dy(1:Ns,:)/sqrt(pen));nd=diag(Dy(Ns+1:end,:)'*Dy(Ns+1:end,:));
