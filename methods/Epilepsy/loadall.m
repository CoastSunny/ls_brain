...
for ci=Itest
    ...
    slicespikes
    data(ci).X=cat(3,iEEG,riEEG);
    data(ci).fX=cat(4,fiEEG,friEEG);
    data(ci).Y=[ones(1,size(iEEG,3)) -ones(1,size(riEEG,3))];
    data(ci).S=[spikes(2,:) ones(1,size(riEEG,3))];
%     fdata(ci).ftiEEG=ftiEEG;
%     fdata(ci).rftiEEG=rftiEEG;
%     fdata(ci).spikes=spikes;
