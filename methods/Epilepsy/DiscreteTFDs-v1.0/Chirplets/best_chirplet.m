...
OPTIONS = optimset('Display', 'none', 'GradObj', 'on', 'GradConstr', 'on');

z = [];
P = fmincon(@(x)chirpfun(x,xx), [Z*M+1+(t-rt) f c d], z, z, z, z, ...
  [1 0 -inf .25], [2*Z*M+1 2*pi inf N/2],@(x)chirpcon(x,xx),OPTIONS);
